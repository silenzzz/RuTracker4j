...
import dev.silenzzz.rutracker.api.constant.ErrorMessage;
import dev.silenzzz.rutracker.api.constant.PageURL;
import dev.silenzzz.rutracker.api.constant.Query;
import dev.silenzzz.rutracker.api.exception.RuTrackerAuthenticationException;
...
import dev.silenzzz.rutracker.api.exception.RuTrackerIncorrectCaptchaException;
...
public class JSoupHttpClient {
    ...
    private Response call(String url, Method method, Map<String, String> payload) throws IOException {
        ...
        if (response.statusCode() != 200) { // He always returns 200 if authenticated
    ...
    private void authenticate() throws RuTrackerException {
        ...
        try {
            ...
            if (!url.equals(PageURL.INDEX.getValue()) && !url.equals(PageURL.LOGIN.getValue())) {
            ...
            if (cookies.isEmpty() && cookieCache.isEmpty()) {
                String errorMessage = Query.LOGIN_ERROR_MESSAGE.getSelectedElementsValue(document.getAllElements());
